...
response = requests.get(STOCK_ENDPOINT, params=stock_dict)
response.raise_for_status()
stocks = response.json()["Time Series (Daily)"]
data_list = [value for (key, value) in stocks.items()]
...
diff_precentage = (difference / yesterday_closing_price) * 100
...
forrmated_ariticles = [f"Headline: {article['title']}.\nBrief: {article['description']}" for article in three_articles]
